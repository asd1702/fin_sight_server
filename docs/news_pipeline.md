...
## 실패/재시도 정책(현재 구현 기준)
- 수집(크롤링) 단계
	- 네이버 검색 메타를 받아온 뒤 원문 크롤링(newspaper3k)에서 오류가 나거나 본문이 너무 짧으면(기본 200자) 해당 기사는 저장하지 않고 건너뜁니다. 즉, 이 경우 DB에 레코드가 생기지 않으므로 상태값(FAILED)도 없습니다.
	- 다음 실행 때 동일/유사 키워드 검색 결과에 다시 노출되면 재시도됩니다(명시적 재시도 횟수는 없음).
- 가공(LLM) 단계
	- 처리 대상 선정은 PENDING + FAILED입니다. get_pending_articles가 두 상태를 함께 조회하므로, 이전 실행에서 FAILED였던 기사도 다음 실행 시 자동으로 재처리 대상이 됩니다.
	- 처리 중 OpenAI/요청 예외 발생, 또는 분석 결과가 유효하지 않은 경우 롤백 후 상태를 FAILED로 업데이트합니다. 다음 실행에서 다시 시도합니다.

### API 호출/크롤링 타임아웃·재시도 설정
- 네이버 뉴스 Open API: 요청 타임아웃 10초, 재시도 없음(에러 시 빈 목록 반환).
- 신문 본문 크롤링(newspaper3k): 요청 타임아웃 10초. 파싱 실패 시 해당 URL은 건너뜁니다.
- OpenAI Chat Completions: SDK 레벨 max_retries=2, timeout=20초. SDK의 자동 재시도 이후에도 실패하면 None으로 처리되어 FAILED가 됩니다.
